<template>
  ...
  >
    ...
    <view class="u-icon-wrap" @tap.stop>
      <view
        class="iconfont icon-close_light u-close-icon"
        v-if="closeable"
        :style="[
          iconStyle,
 ...
 * @property {String} text 标签字体颜色，默认为空字符串，即不处理
 * @property {String} bg-color 自定义标签的背景颜色
 * @property {String} color 自定义标签的背景颜色
 ...
 * @property {Boolean} disabled  是否禁用这个标签，禁用的话，会屏蔽点击事件
 * @property {Boolean} closeable 是否可关闭，设置为true，文字右边会出现一个关闭图标（默认false）
 * @property {Boolean} show 标签显示与否（默认true）
 * @property {Function} click 点击标签触发
 * @property {Function} close closeable为true时，点击标签关闭按钮触发
...
export default {
  ...
  computed: {
    ...
    iconStyle() {
      ...
      if(this.mode === 'plain' || this.mode === 'light') style.color = this.getIconColor(this.type);
    ...
    closeIconColor() {
      ...
      else return this.getIconColor(this.type);
    }
  },
  methods: {
    getIconColor(type) {
