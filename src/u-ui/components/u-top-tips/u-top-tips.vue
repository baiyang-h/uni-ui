...
/**
 * topTips 顶部提示
 * @description 该组件一般用于页面顶部向下滑出一个提示，而后自动收起的场景
 * @property {String, Number} height 导航栏高度(包含状态栏高度在内)，单位rpx
...
export default {
  ...
  data() {
    return {
      ...
      title: '',          // 组件中显示的消息内容
      type: 'primary',    // 消息的类型（颜色不用），primary、success、error、warning、info
      duration: 1000,     // 组件显示的时间，单位为毫秒
    }
  },
  computed: {
    uZIndex() {
