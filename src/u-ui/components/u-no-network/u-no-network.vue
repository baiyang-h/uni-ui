<template>
  ...
  >
    <image class="u-error-icon" :src="image" mode="widthFix" />
    <view class="u-tips">
      {{tips}}
    </view>
 ...
 * @property {String} tips 没有网络时的提示语（默认，网络信息丢失）
 * @property {String Number} zIndex 组件的z-index值（默认1080）
 * @property {String} image 无网络的图片提示，可用的src地址或base64图片
 * @event {Function} retry 重试，用户点击页面的重试按钮时触发
 * @example <u-no-network />
 */
export default {
  name: "u-no-network",
  props: {
  ...
  mounted() {
    ...
    uni.onNetworkStatusChange((res) => {
      this.isConnected = res.isConnected;    // 当前是否有网络连接
      this.networkType = res.networkType;    // 网络类型
    });
    uni.getNetworkType({
      success(res) {
