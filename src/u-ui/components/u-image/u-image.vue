...
/**
 * Image 图片
 * @description 此组件为uni-app的加强版，在继承了原有的功能外，还支持了、加载中、加载失败提示、圆角和形状等
 *
 ...
 * @property {Boolean} lazy-load 是否懒加载，仅微信小程序、App、百度小程序、字节跳动小程序有效（默认 true）（官网）
 * @property {Boolean} show-menu-by-longpress 开启长按图片显示识别小程序码菜单（官网）
 * @property {String} loading-icon 加载中的图标，或者小图片
 * @property {String} error-icon 加载失败的图标，或者小图片
 ...
 * @property {String} duration 搭配fade参数的过渡时间，单位ms（默认 500）
 * @property {String} webp 默认不解析 webP 格式，只支持网络资源（官网）
...
export default {
  ...
  watch: {
    src: {
      ...
      handler(n) {
        if(!n) {
          // 如果传入null、''、false、undefined，标记为错误状态
          this.isError = true;
          this.loading = false;
        } else {
          this.isError = false;
