...
/**
 * line 线条
 * @description 此组件一般用于显示一根线条，用于分隔内容块，有横向和竖向两种模式，且能设置 0.5px 线条，使用也很简单
 * 默认是0.5px，如果不选择使用细线hairLine=false，则就是1px
 *
 * @property {String} color 线条的颜色(默认#e4e7ed)
 * @property {String} length 长度，竖向时表现为高度，横向时表现为长度，可以为百分比，带rpx单位的值等，如果直接写数字，则单位默认是rpx
 * @property {String} direction 线条的方向，row-横向，col-竖向(默认row)
 * @property {String} border-style 线条的类型，solid-实线，dashed-方形虚线，dotted-圆形虚线（默认solid）
  ...
  computed: {
    lineStyle() {
      ...
      if(this.direction === 'row') {
        ...
        // 如果是竖向竖向，边框宽度为1px，再通过transform缩小一半，就是0.5px了
