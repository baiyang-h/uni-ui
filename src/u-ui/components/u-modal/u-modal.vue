<template>
  <view>
    ...
    >
      <view class="u-modal">
        <view v-if="showTitle" class="u-modal__title u-line-1" :style="[titleStyle]">{{ title }}</view>
        <view class="u-modal__content">
          <view :style="[contentStyle]" v-if="$slots.default || $slots.$default">
            <slot />
          </view>
          <view v-else class="u-modal__content__message" :style="[contentStyle]">{{ content }}</view>
        </view>
        <view class="u-modal__footer u-border-top" v-if="showCancelButton || showConfirmButton">
          <view
            v-if="showCancelButton"
            :hover-stay-time="100"
            hover-class="u-modal__btn--hover"
            class="u-modal__footer__button"
          ...
          <view
            ...
            :hover-stay-time="100"
            :hover-class="asyncClose ? 'none' : 'u-modal__btn--hover'"
            class="u-modal__footer__button hairline-left"
            :style="[confirmBtnStyle]"
            @tap="confirm"
          >
            <slot v-if="$slots['confirm-button']" name="confirm-button"></slot>
            <block v-else>
              <view class="iconfont icon-loading1" :color="confirmColor" v-if="loading"></view>
              <block v-else>
                {{confirmText}}
              </block>
            </block>
 ...
 * @property {Boolean} async-close 是否异步关闭，只对确定按钮有效
 ...
 * @property {String, Number} border-radius 模态框圆角值，单位 rpx（默认16）
 ...
 * @property {Function} confirm 确认按钮被点击
 * @property {Function} cancel 取消按钮被点击
 * @example <u-modal :title="title" :content="content"></u-modal>
 */
export default {
  name: "u-modal",
  props: {
  ...
  watch: {
    // 如果是异步关闭时，外部修改v-model的值为false，重置内部的loading状态
    // 避免下次打开的时候，状态混乱
    value(v) {
