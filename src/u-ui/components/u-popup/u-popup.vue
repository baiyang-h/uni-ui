    ...
    <!----------   popup 是 center 情况   ---------->
      ...
      >
        <view
          ...
          :class="['u-close--' + closeIconPos, closeIcon]"
          :style="{
 ...
 * @property {String} close-icon 关闭图标的icon
 * @property {String} close-icon-pos 自定义关闭图标的位置（默认top-right）
 * @property {String} close-icon-color 自定义关闭图标的颜色（默认#909399）
 * @property {Number, String} close-icon-size 关闭图标大小，单位rpx（默认30）
 *
 * @property {Boolean} mask 是否显示遮罩（默认true）
 * @property {Boolean} mask-close-able 点击遮罩是否可以关闭弹出层（默认true）
 * @property {Object} mask-custom-style 遮罩的样式，一般用于修改遮罩的透明度
 * @property {String, Number} duration 遮罩打开或收起的动画过渡时间，单位ms
 *
 * @event {Function} open 弹出层打开
 * @event {Function} close 弹出层关闭
 * @example <u-popup v-model="show"><view>这是内容</view></u-popup>
 */
export default {
  name: "u-popup",
  props: {
  ...
  data() {
    return {
      ...
      showDrawer: false,
      timer: null,
      // 主要是为了不让 watch 监听 关闭时重复触发，只触发一次
      closeFromInner: false, // value的值改变，是发生在内部还是外部（即内部自己点击关闭是内部，外部prop的value改变是外部发生）
    }
  },
  computed: {
  ...
  methods: {
    ...
    // 此处的原理是，关闭时先通过动画隐藏弹框和遮罩，再移除整个组件
    // 打开时，先渲染组件，延时一定时间再让遮罩和弹框的动画起作用
    // 所以注意看，open和close时传入的param1和param2，visibleSync、showDrawer顺序是不一样的
    ...
    // 判断传入的值，是否带有单位，如果没有，就默认用rpx
    getUnitValue(val) {
      return this.$u.addUnit(val)
    }
  }
}
</script>
