...
/**
 * u-divider 分割线
 * @description 区域内容的分割线，一般用于页面底部"没有更多"的提示
 *
 ...
 * @property {String Number} 文字大小，单位rpx（默认26）
 * @property {String} bg-color 整个divider的背景颜色（默认#fff）
 * @property {String} height 整个divider的高度，单位rpx（默认40）
 * @property {String} type 将线条设置为主题色（默认primary）
 * @property {Boolean} useSlot 是否使用slot传入内容，如果不传入，中间不会有空隙（默认true）
 * @property {String Number} margin-top 与前一个组件的距离，单位rpx（默认0）
 * @property {String Number} margin-bottom 与后一个组件的距离，单位rpx（默认0）
 * @property {Function} click divider组件被点击时触发
...
export default {
  ...
  computed: {
    lineStyle() {
      let style = {};
      style.width = this.$u.addUnit(this.halfWidth);
