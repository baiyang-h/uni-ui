...
export default {
  ...
  methods: {
    ...
    click(e) {
      ...
      this.$u.throttle(() => {
        // 如果按钮是disabled和loading状态，不触发水波纹效果
    ...
    getWaveQuery(e) {
      this.getElQuery().then(data => {
        ...
        if (!data.targetWidth) return false;
        this.fields = data;
        let touchesX = '',
            touchesY = '';
        // #ifdef MP-BAIDU
    ...
    getElQuery() {
      return new Promise(resolve => {
        ...
        //#endif
        queryInfo.select('.u-btn').boundingClientRect(data => {
          resolve(data)
        }).exec();
      })
    },
...
@import "~u-ui/styles/style.components.scss";
.u-btn::after {
  border: none;
}
.u-btn {
  ...
  &--default {
    ...
    border-color: $u-main-border-color;
...
// hover-class 点击按钮时的样式
// plain 镂空时 和 不镂空时
.u-default-plain-hover {
  color: $u-type-primary-dark !important;
  background: $u-type-primary-light !important;
}
.u-primary-plain-hover, .u-primary-hover {
  color: #ffffff !important;
  background: $u-type-primary-dark !important;
}
.u-success-plain-hover, .u-success-hover {
  color: #ffffff !important;
  background: $u-type-success-dark !important;
}
.u-warning-plain-hover, .u-warning-hover {
  color: #ffffff !important;
  background: $u-type-warning-dark !important;
}
.u-error-plain-hover, .u-error-hover {
  color: #ffffff !important;
  background: $u-type-error-dark !important;
}
.u-info-plain-hover, .u-info-hover {
  color: #ffffff !important;
...
.u-wave-ripple {
  position: absolute;
  z-index: 0;
  border-radius: 100%;  // >50% 都一样
  background-clip: padding-box;
  pointer-events: none;  // 阻止点击触发状态变化、触发事件等，即阻止用户点击动作产生任何效果
